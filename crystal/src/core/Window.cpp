...
static void CreateCustomDecoration(GLFWwindow* window)
{
    MARGINS  dwm_margins = {1, 0, 0, 1};
    HWND     hwnd        = glfwGetWin32Window(window);
    LONG_PTR lStyle      = GetWindowLongPtr(hwnd, GWL_STYLE);
    // lStyle               &= ~(WS_SYSMENU);
    SetWindowLongPtr(hwnd, GWL_STYLE, lStyle);
    DwmExtendFrameIntoClientArea(hwnd, &dwm_margins);
    BOOL t = TRUE;
    DwmSetWindowAttribute(hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE, &t, sizeof(BOOL));
    RECT window_rect;
    GetWindowRect(hwnd, &window_rect);
    SetWindowPos(hwnd, NULL, 0, 0, window_rect.right - window_rect.left, window_rect.bottom - window_rect.top,
...
namespace crystal
{
    Window::Window(const char*           name,
        ...
        : m_window_name(name),
          ...
          m_window_destroyed_b(false),
          m_window_visible_b(false),
          m_current_fps(0)
    {
    ...
    void Window::show()
    {
        ...
        m_glfw_window = glfwCreateWindow(m_frame_buffer_size.width, m_frame_buffer_size.height, m_window_name.data(),
                                         NULL, NULL);
        ...
        CreateCustomDecoration((GLFWwindow*) m_glfw_window);
        ...
        while (m_renderer_attached_b.load())
        {
            ...
            if (timespent_nanos >= one_second_in_nanos)
            {
                logger::Info(std::string("FPS: ") + std::to_string(frames_rendered)
                             + " | Timespent: " + std::to_string(timespent_nanos) + "ns");
