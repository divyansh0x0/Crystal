...
static DWORD getWindowStyle(const _GLFWwindow* window)
{
    ...
    else
    {
        ...
        if (window->decorated)
        {
            ...
            style |= WS_POPUP | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_CAPTION; // Modified
    }
...
void _glfwSetWindowIconWin32(_GLFWwindow* window, int count, const GLFWimage* images)
{
    ...
    if (count)
    {
        const GLFWimage* bigImage   = chooseImage(count, images, GetSystemMetrics(SM_CXICON),
                                                  GetSystemMetrics(SM_CYICON));
        const GLFWimage* smallImage = chooseImage(count, images, GetSystemMetrics(SM_CXSMICON),
...
void _glfwSetWindowMonitorWin32(
...
GLFWbool
    _glfwGetPhysicalDevicePresentationSupportWin32(VkInstance instance, VkPhysicalDevice device, uint32_t queuefamily)
{
