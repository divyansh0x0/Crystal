...
8.  **`GL_CALL` macro:** This is likely a custom macro used by the `ktl::graphics` library or the developer to wrap OpenGL function calls, possibly for error checking.
...
8.  **`ktl::graphics::GLShader my_shader (vert, frag);`**
    * **Name Breakdown:**
        * `ktl::graphics::GLShader`: This is a class named `GLShader` within the `graphics` namespace, which itself is within the `krystal` namespace. This suggests it's part of a custom graphics library.
...
11. **`my_shader.activate ();`**
    * **Function Name:** `activate` (a member function of the `ktl::graphics::GLShader` class)
...
12. **`shaderID = my_shader.getID ();`**
    * **Function Name:** `getID` (a member function of the `ktl::graphics::GLShader` class)
